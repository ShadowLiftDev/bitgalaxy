...
export async function POST(req: NextRequest) {
  try {
    ...
    const stats = body.stats as
      | { hits?: number; timeMs?: number; maxSpeed?: number }
      | undefined;
    ...
    // ✅ AUTH: always derive the player id from the authed user
    const acting = await requireUser(req);
    const targetUserId = acting.uid;

    // ✅ Backward compatibility: if old client still sends userId, enforce match
    if (body.userId && String(body.userId) !== targetUserId) {
      return NextResponse.json(
        { error: "You can only submit your own Galaxy Paddle run." },
        { status: 403 },
      );
    }
    ...
    await adminDb.runTransaction(async (tx) => {
      ...
      const timeMs = typeof stats?.timeMs === "number" ? stats.timeMs : null;
      const maxSpeed =
        ...
        playerRef,
        {
          ...
          specialEvents: {
            ...
            galaxyPaddle: {
              ...
              lastResult: { level: requestedLevel, hits, timeMs, maxSpeed },
            },
          },
          updatedAt: now,
        },
    ...
    if (error?.message === "GALAXY_PADDLE_TIER_ALREADY_RECORDED") {
      return NextResponse.json(
        {
          error:
